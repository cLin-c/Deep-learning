...
# 在所有 import 之后、新建类定义之前插入 ↓
# from mamba_ssm import Mamba          # 已经 import 过可跳过
import torch.nn.functional as F

class Mamba2d(nn.Module):
    """把 BCHW ↔ BLC 的排布转换给 Mamba 使用"""
    def __init__(self, d_model, d_state=16, d_conv=4, expand=2, **kw):
        super().__init__()
        self.m = Mamba(d_model=d_model, d_state=d_state,
                       d_conv=d_conv, expand=expand, **kw)

    def forward(self, x):
        b, c, h, w = x.shape          # BCHW ➜ B L C
        x = x.flatten(2).transpose(1, 2)       # B, L=H*W, C
        x = self.m(x)                           # SSM
        return x.transpose(1, 2).view(b, c, h, w)   # 还原

class MambaBlock(nn.Module):
    def __init__(self, module1, dim, module2):
        super().__init__()
        self.block = nn.Sequential(
            module1,
            LayerNorm([dim, 1, 1]),  # 注意维度是否匹配
            Mamba(dim=dim, depth=2, bidirectional=True),
            module2
        )
        self.residual = nn.Identity()  # 默认无残差或用 skip connection
            ...
            if isinstance(m, Detect):
                ...
                for seq in m.cv2:
                    ...
                    new_seq = nn.Sequential(
                        modules[0],
                        Mamba2d(d_model=dim, d_state=16, d_conv=4, expand=2),  # ★ 新
                        modules[2]
                    )
